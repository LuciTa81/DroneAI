...
1. **G0 — source and data audit:** verify project/upstream commits, raw Train/Test
   counts, annotation pairing, ImageNet backbone SHA-256, official checkpoint
   SHA-256, container digest, GPU, and storage.
2. **G1 — evaluator reproduction:** prepare an isolated official Test adapter and
   evaluate the pinned official checkpoint on all 334 images. Proceed only if the
...
3. **G2 — Train/Test preparation:** convert all 1,201 Train and 334 Test samples,
   write `train.txt` and `test.txt`, verify disjoint official partitions, and save
   content-addressed input/output inventories.
4. **G3 — synthetic and one-update smoke:** verify imports, CUDA matmul, one
   optimizer update, finite loss, checkpoint round-trip, and manifest reload.
5. **G4 — one-epoch smoke:** train one full epoch, run the approved official Test
   evaluation cadence, and verify all 334 observations and restart evidence.
6. **G5 — 800-epoch reproduction:** launch a fresh FP32 run from the pinned
   ...
   completed boundary.
7. **G6 — final comparison:** reload best-MAE, best-RMSE, and epoch-800
