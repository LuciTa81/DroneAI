"""Run one frozen CSRNet validation sample before the 36-image benchmark."""
...
from droneai.csrnet_adapter import CSRNetAdapter
from droneai.csrnet_smoke import (
...
def compatibility_checks(
    ...
    density_valid = bool(
        density_present and np.isfinite(density).all() and np.all(density >= 0)
    )
    mass_matches = bool(
        density_valid
        and math.isclose(
            float(density.sum(dtype=np.float64)),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    config = load_csrnet_smoke_config(paths["config"])
    validate_csrnet_rights_decision(
        paths["rights"],
    ...
    validate_csrnet_upstreams(
        ...
        expected_split_commit=str(config["split_upstream_commit"]),
    )
    ...
    result = {
        ...
        "native_density_shape": None
        if prediction.density is None
        else list(np.asarray(prediction.density).shape),
    ...
    print(
        f"CSRNet one-sample {result['status']}: sample={sample.sample_id} "
        f"gt={sample.ground_truth_count} pred={prediction.predicted_count} "
        f"score={technical_score:.1f}"
    )
