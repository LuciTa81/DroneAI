"""Run the frozen 36-image CSRNet UCF-QNRF validation benchmark."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the frozen 36-image UCF-QNRF validation benchmark with the "
            "rights-gated ShanghaiTech-A CSRNet checkpoint."
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if expected_samples != 36 or len(prepared.samples) != 36:
        raise ValueError(
            "CSRNet benchmark requires exactly 36 frozen validation samples; "
    ...
    protocol = build_csrnet_protocol(
        config,
