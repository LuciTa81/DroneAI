"""Run one frozen MPCount validation sample before the 36-image benchmark."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one frozen MPCount validation sample."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        f"MPCount one-sample {result['status']}: sample={sample.sample_id} "
