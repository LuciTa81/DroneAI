...
def _parser() -> argparse.ArgumentParser:
    ...
    a1 = subparsers.add_parser(
        "A1", help="fresh one-epoch Train1201 and complete official Test334 gate"
    )
    a1.add_argument("--config", type=Path, required=True)
    a1.add_argument("--run-id", required=True)
    a1.add_argument("--processed-root", type=Path, required=True)
    a1.add_argument("--g2-manifest", type=Path, required=True)
    a1.add_argument("--upstream-dir", type=Path, required=True)
    a1.add_argument("--backbone", type=Path, required=True)
    a1.add_argument("--container-image-digest", required=True)
    a1.add_argument("--device", default="cuda:0")
    return parser
...
def _run_a1(args: argparse.Namespace) -> int:
    profile = load_official_training_data_profile(args.config)
    processed = args.processed_root.resolve(strict=True)
    expected_processed = Path(str(profile.processed_root)).resolve(strict=True)
    if processed != expected_processed:
    ...
    config, upstream_audit = synthesize_a1_config(
        profile,
        upstream_dir=args.upstream_dir,
        processed_root=processed,
        backbone_path=args.backbone,
    ...
    project_root = Path(__file__).resolve().parents[1]
    project_commit, untracked = _project_identity(project_root)
    runtime = TorchOfficialA1Runtime(
        profile,
        config=config,
        upstream_dir=args.upstream_dir,
        backbone_path=args.backbone,
