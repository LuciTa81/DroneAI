"""Run one explicit frozen STEERER validation sample for report evidence."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run one explicit frozen STEERER validation sample")
    ...
    parser.add_argument("--long-side-cap", type=int, default=3072)
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    config = load_steerer_smoke_config(paths["config"])
    validate_steerer_rights_decision(
        paths["rights"],
    ...
    adapter = STEERERAdapter(
        ...
        device=args.device,
        long_side_cap=args.long_side_cap,
    )
    brief = adapter.brief()
    ...
    print(
        f"STEERER one-sample {result['status']}: sample={sample.sample_id} "
