"""Research-only PET wrapper around the frozen DM-Count QNRF validation split."""
...
PET_CANDIDATE_ID = "pet-official-ucf-qnrf-academic-research-comparison"
PET_PINNED_COMMIT = "5b4dd7da8b11568a3305a88bb7c99a7fc831a998"
...
_PET_CONFIG_OVERRIDES: dict[str, object] = {
    "run_id": "pet-ucf-qnrf-validation-smoke",
    "protocol_id": "pet-official-qnrf-val-smoke-v1",
    "model_id": "pet-official-ucf-qnrf",
    "upstream_commit": PET_PINNED_COMMIT,
    "split_upstream_commit": SPLIT_PINNED_COMMIT,
    "candidate_id": PET_CANDIDATE_ID,
    ...
    "checkpoint_split_evidence": (
        ...
        "preprocessing exposes only original UCF-QNRF Train and Test partitions."
    ),
}
...
def _expected_config() -> dict[str, object]:
    ...
    expected.update(_PET_CONFIG_OVERRIDES)
    targets = dict(cast(dict[str, object], expected["targets"]))
    targets.update(
        {
...
def load_pet_smoke_config(path: str | Path) -> dict[str, object]:
    payload = _object(path, "PET smoke config")
    ...
    if payload != expected:
        ...
        raise ValueError(
            "PET config must preserve the frozen validation protocol; "
...
def _component_ids(manifest: dict[str, object]) -> dict[str, str]:
    ...
    for component in components:
        ...
        kind, identity = component.get("component_type"), component.get("component_id")
        if (
...
def validate_pet_rights_decision(
    ...
    *,
    ...
    if (
        ...
        raise PermissionError("PET rights decision candidate or manifest binding failed")
    if decision.get("status") != "PASS_RESEARCH_ONLY":
        raise PermissionError("PET evaluation requires PASS_RESEARCH_ONLY")
    actions = decision.get("allowed_actions")
    if not isinstance(actions, list) or any(not isinstance(item, str) for item in actions):
        raise PermissionError("PET rights actions must be a string list")
    if REQUIRED_ACTION not in actions:
        raise PermissionError(f"PET rights must authorize {REQUIRED_ACTION}")
    forbidden = sorted(_FORBIDDEN.intersection(actions))
    if forbidden:
        raise PermissionError("PET research lane cannot authorize " + ", ".join(forbidden))
    return decision


def validate_pet_split_upstream(
    *,
    ...
    if expected_commit != SPLIT_PINNED_COMMIT:
        raise ValueError("PET split upstream must use the pinned DM-Count commit")
    validate_official_split_paths(
        upstream_dir=root,
    ...
    if _git_head(root) != expected_commit or _git_status(root):
        raise ValueError("PET split upstream must be clean at the pinned commit")
...
def write_split_source_manifest(
    ...
    *,
    ...
    if model_upstream_commit != PET_PINNED_COMMIT:
        raise ValueError("PET model upstream commit is not pinned")
    if split_upstream_commit != SPLIT_PINNED_COMMIT:
        raise ValueError("PET split upstream commit is not pinned")
...
def build_pet_protocol(
    ...
    *,
    ...
    if config != _expected_config():
        raise ValueError("PET protocol config is not frozen")
