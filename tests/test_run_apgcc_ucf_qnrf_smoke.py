...
from scripts import run_apgcc_ucf_qnrf_smoke as runner
...
def test_fake_36_sample_run_delegates_to_common_harness(
    ...
    config = {
        ...
        "candidate_id": "apgcc-official-shha-to-ucf-qnrf-research-comparison",
        "expected_samples": 36,
    }
    ...
    monkeypatch.setattr(runner, "load_apgcc_smoke_config", lambda _path: config)
    monkeypatch.setattr(
        runner,
    ...
    monkeypatch.setattr(runner, "APGCCAdapter", FakeAdapter)
    monkeypatch.setattr(
        runner,
    ...
    assert calls["adapter"] == {
        ...
        "expected_upstream_commit": APGCC_PINNED_COMMIT,
...
def test_wrong_sample_count_stops_before_adapter(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    output = tmp_path / "run"
    config = {
    ...
    monkeypatch.setattr(runner, "load_apgcc_smoke_config", lambda _path: config)
    monkeypatch.setattr(runner, "validate_apgcc_rights_decision", lambda *a, **k: None)
    monkeypatch.setattr(runner, "validate_apgcc_split_upstream", lambda **kwargs: None)
    monkeypatch.setattr(
        runner,
        "prepare_smoke_samples",
